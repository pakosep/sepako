...
volatile uint32_t  tim0,tim1,tim2,tim3,tim4,tim5,delay_us;
volatile uint32_t  Tim6,Time6,Timt,Timt3;
...
u08 *p_TSpm = (u08 *)&TSpm;	// wskaŸnik na strukture do odczytu z karty pamiêci
...
void data_logger_chart( T_SD1 *,T_CHART_PARAM *,T_CHART_DATA *,u08 );
void data_looger_tool ( T_SD1 *sd,T_CHART_PARAM *chart,char *ch);
void data_looger_tool_menu( void);
void ramka(void);
...
int main(void){
		...
		SysInit();
		#define dly1 0xffffff
		
		// #define BLACK_Board
		#ifdef BLACK_Board
		 #define LED PB12_o
		 GPIOB->CRH = (GPIOB->CRH & 0xfff0ffff) | 0x00010000;	// LED Black PB12
		#else
		 #define LED PC13_o
		...
		wart[5][5] = 15;
		uint16_t *BKPSRAMADDR0 = (uint16_t *)0x40024000UL;//wskaŸnik do pocz¹tku przestrzeni adresowej BACKUP SRAM
		*(BKPSRAMADDR0 + 5) = wart[5][5];
		BKPSRAMADDR0 				= &wart[5][5];
		//wart[licz1][licz2]= *(BKPSRAMADDR0 + licz3);
		*/
		
		Usart1Init(); // Green->PA10, White->PA9 
		head();
...
void IRQLoraInit	(void)			{
		/*
			...
			Jednoczeœnie pin moze byc skonfigurowany tylko dla jednego portu
		 */
...
void LoRaRX(void){
		...
		//Lora2 PA2=MOSI, PA1=MISO,PA0=SCK,PB8=CE,PB9=RST 
		/* // SPI Configured 
		GPIOB->CRH = (GPIOB->CRH & 0xffffff00) | 0x00000033;	//   
		GPIOA->CRL = (GPIOA->CRL & 0x00000000) | 0xBBB33343;	//   PA7=MOSI, PA6=MISO, PA5=SCK, PA4=CE PA3=Reset
		RX1276_RST1 = 1;
		RX1276_RST2 = 1;
		//GPIOC->CRH = (GPIOC->CRH & 0xfff0ffff) | 0x00010000;	// enc28_CS
		
		RCC->APB2ENR   |= RCC_APB2ENR_SPI1EN;
		SPI1->CR1 = SPI_CR1_SSM|SPI_CR1_SSI|SPI_CR1_MSTR|SPI_CR1_BR_0*7;	// 0x037c		
		...
		*/
		...
		#ifdef NONE
		GPIOB->CRH = (GPIOB->CRH & 0xffffff00) | 0x00000043;	//   
		GPIOA->CRH = (GPIOA->CRH & 0x0ff00ff0) | 0x30034003;	
		
		SX1278_hw1.mosi = (u32 *)m_BITBAND_PERIPH(&GPIOA->ODR, 12); 
		SX1278_hw1.miso = (u32 *)m_BITBAND_PERIPH(&GPIOA->IDR, 11); 
		SX1278_hw1.sck  = (u32 *)m_BITBAND_PERIPH(&GPIOA->ODR,  8); 
		SX1278_hw1.nss  = (u32 *)m_BITBAND_PERIPH(&GPIOA->ODR, 15); 		
		SX1278_hw1.rst  = (u32 *)m_BITBAND_PERIPH(&GPIOB->ODR, 8); 
		SX1278_hw1.dio0 = (u32 *)m_BITBAND_PERIPH(&GPIOB->IDR, 9); 		
		
		SX1278_1.hw = &SX1278_hw1;			// wybor modulu
		SX1278_begin(&SX1278_1, SX1278_433MHZ, SX1278_POWER_11DBM, SX1278_LORA_SF_11,SX1278_LORA_BW_125KHZ);
		...
		//SX1278_sleep(&SX1278);
		//SX1276Write(reg, 0x08);
		
		//UaPutK("\r\n Reg1 0x");		hex2uart(reg,2);
		//UaPutK("= 0x");					  
		//SX1278.hw = &SX1278_hw2;			
		//dat = SX1278_SPIRead(&SX1278, reg);
		//SX1276Read(reg, &dat);
		//hex2uart(dat,2);
		
		int 	message_length=65,ret;		
		static char 	buf_tx[256],buf_rx[256],str_num[12],buf2[10];
		s16 prssi,n=0;
		...
		while(1){
			...
			if(p_Fifo1->rct){
				...
				switch (swh)	{
					...
					UaPutK("\r\n");	sint2uart(message_length);
					
					strcat (buf_tx, uint2str(message_length,str_num));
					
					//ret = SX1278_LoRaEntryRx(&SX1278_2, message_length, 1000);					
					UaPutK("\r\n Entry Rx status=");	hex2uart(ret,2);
					
					ret = SX1278_LoRaEntryTx(&SX1278_1, message_length, 1000);
					
					UaPutK("\r\n ret1=0x");			hex2uart(ret,1);
					ret = SX1278_LoRaTxPacket(&SX1278_1, (uint8_t *) buf_tx, message_length,4000);
					
					UaPutK("\r\n ret2=0x");			hex2uart(ret,1);
...
void LoRaTX(void){
		...
		int 	message_length=65,ret;		
		static char 	buf_tx[256],buf_rx[256],str_num[12];
		s16 prssi,n=0;
		u32 tim=tim,uxt,uxn;
		...
		strcat (buf_tx," ");
		SX1278_LoRaEntryTx (&SX1278_1, strlen(buf_tx), 2000);
		SX1278_LoRaTxPacketStart(&SX1278_1, (uint8_t *) buf_tx, message_length);  // Start transmisji
		flag_9=1;
		
		//GPIOA->CRL = (GPIOA->CRL & 0xfffff000)|0x00000222 ;
		GPIOA->CRH = (GPIOA->CRH & 0xfff0f0ff)|0x00020200 ; 
		...
		while(1){
			...
			// ==== Koniec transmisji ====
			/*
			if((*SX1278_hw1.dio0) && flag_9){
				
				SX1278_LoRaTxPacketEnd(&SX1278_1);
				flag_9=0;
				TIM_stop(&tim);
				//UaPutK("\r\n TX[ms]=");
				//uint2uart(tim/TIM_MS);
				//UaPutC(' ');
				//UaPutS(buf_tx);
				//UaPutC(' ');
			...
			if((*SX1278_hw1.dio0) && flag_9){
				...
				TIM3_stop(&tim);
				UaPutK("Sleep ");
				uint2uart(tim/TIM3_MS);	UaPutK("[ms]");
				SX1278_sleep(&SX1278_1);
				
				//while(!bRTC_CRL_RTOFF); hex2uart(RTC->CRL,4);	  	//wait to synchro
				//uint2uart(tim/TIM3_US);	UaPutK("[us]");
				/* while(!bRTC_CRL_RTOFF); 	//wait to synchro
				bRTC_CRL_CNF = 1;					//Start edit mode
				RTC->CRL &= ~RTC_CRL_ALRF;
				bRTC_CRL_CNF = 0;					//End edit mode
				...
				*/
				...
				SX1278_LoRaEntryTx (&SX1278_1, message_length, 2000);
				strcpy (buf_tx," ");		
				strcat (buf_tx,unt2str(n++,str_num,4));
				strcat (buf_tx," ");
				message_length = strlen(buf_tx); 
				TIM3_start();
				SX1278_LoRaTxPacketStart(&SX1278_1, (uint8_t *) buf_tx, message_length);  // Start transmisji
			...
			//trg1 = SysTick->VAL;
			if(TIM3_MS*2000 < (t3rg1 - TIM3->CNT)){
				t3rg1 = TIM3->CNT;
				//uxt = RTC->CNTL+(RTC->CNTH<<16);
				//uint2uart(uxt);
			}
			if(TIM3_MS*200 < (t3rg2 - TIM3->CNT)){
				t3rg2 = TIM3->CNT;
			}
		}
	}
...
//=========================================================================================================
void data_logger(void)	{		// Odczytywanie danych z karty SD
	...
	#undef wYmin 
	#define wYmin		14 				//14 			dolna granica wykresu Y
	#undef wYmax 
	#define wYmax		(239-12) 	//240-13 	górna granica wykresu Y
	
	#undef wXlen
	#define wXlen		((wXmax)-(wXmin)+(1)) 	//szerokoœæ wykresu 
	
	#define VerB    (((wYmax)-(wYmin))/2)		// symetralna pionowa (Vertical Bisector)
	//#define HorB    ((wXlen)/2) 					// symetralna pozioma (Horizontal Bisector)
	#define HorB    (144*2) 								// symetralna pozioma (Horizontal Bisector)

	#define DATA_LOGGER_STIME 	1518670800ul 
	#define DLD 		1				// Co ile sekund pomiar							
	#define DLS 		1				// ile pomiarów na sekunde
	...
	s16 AMt;
	tim=tim;
	//static volatile u16	dma_buf[4*(wXlen)];	
	static T_CHART_PARAM schp;	// parametry wykresu
	static T_CHART_DATA schd;	// dane wykresu
	static T_SD1 	 sda;		// struktura danych
	char tbuf[9]={"00:00:00"};
	char dbuf[11]={"0000-00-00"};
	char str[11];
	
	#define flg_sec			  		bitband_t m_BITBAND_PERIPH(&BKP->DR8,0)		//2sec interwal flag
	#define flg_refresh				bitband_t m_BITBAND_PERIPH(&BKP->DR8,1)		//refresh flag
	#define flg_chart_change 	bitband_t m_BITBAND_PERIPH(&BKP->DR8,2)		//Change chart  flag
	#define flg_chart_browse 	bitband_t m_BITBAND_PERIPH(&BKP->DR8,3)		//hm flag
	#define flg_wyb						bitband_t m_BITBAND_PERIPH(&BKP->DR8,4)		//choice flag
	#define flg_wr_data				bitband_t m_BITBAND_PERIPH(&BKP->DR8,5)		//
	#define flg_wr_usec				bitband_t m_BITBAND_PERIPH(&BKP->DR8,6)		//
	#define MSTAT		BKP->DR10
	...
	if(dinit )
		...
		UaPutK("\r\n SD Card Init ERROR");
		TFT_txt("SD Card Init ERROR",100, 120,YELLOW);
	}
	rtc_GetTime(1,&Rtc);	
	ramka();

	//p_Fifo1->rct=1;	p_Fifo1->rwi=1;	p_Fifo1->rbuf[0]='a';
	// Parametry poczatkowe przetrzymywane w rejestrach zapasowych
	
	schp.hm= (BKP->DR2<<16) + BKP->DR1;	// przesuwanie poziome
	schp.vm= BKP->DR3;									// przesuwanie pior_dma_bufe
	schp.hs= BKP->DR4;									// skala pozioma
	schp.vs= BKP->DR5; 									// skala pionowa
	schp.sb= 1000;											// blok startowy z karty pamieci
	schp.hb= wXlen;										// blok startowy z karty pamieci
	
	// BKP->DR9 licznik zapisów na karte pamieci
	
	schp.sc=(1<<0)|(1<<1)|(1<<2)|(0<<3);	// który wykres wyœwietlaæ
	
	schp.cc[0]=YELLOW; schp.wd[0]=0; schp.ml[0]=1; schp.dv[0]=1; schp.dp[0]=0; schp.ad[0]= 0;
	schp.cc[1]=CYAN;	 schp.wd[1]=0; schp.ml[1]=1; schp.dv[1]=1; schp.dp[1]=1; schp.ad[1]= -200;
	schp.cc[2]=GREEN;	 schp.wd[2]=0; schp.ml[2]=1; schp.dv[2]=1; schp.dp[2]=1; schp.ad[2]= -200;
	schp.cc[3]=DGRAY;	 schp.wd[3]=0; schp.ml[3]=1; schp.dv[3]=2; schp.dp[3]=0; schp.ad[3]= 10;
	//sch.cc[3]=RED;		sch.wd[3]=0;
	//data_logger_chart(&sda,&schp,&schd,1);
	
	TFT_txt("Adc",0,14*1-2,schp.cc[0]);
	TFT_txt("Hum",0,14*3-2,schp.cc[1]);
	TFT_txt("Tem",0,14*5-2,schp.cc[2]);
	//================================================================================
	while(1){
		//if(0){
		if(p_Fifo1->rct){	// Wywolanie interfejsu szeregowego gdy odebrano znak
			UART_getChar(&chr);
				...
				switch (chr) {
				//======== Obsluga Menu wykresu ======================================
				case '8' : 	{ flg_chart_browse= 0; kier	= 0; }
					// ostatni blok jest pomiarowy					
				break;	// ON refresh;
				
				case '0' :  flg_wyb ^= 1; 	break;
				
				case '1' :  // Zmniejszanie poziome
					if(flg_wyb==0) { schp.hm += 1*schp.hs; flg_chart_browse=1 ; } 
					else  				 { schp.hs = schp.hs > 1 ? schp.hs-1 : 1;     }		// Skala pozioma hs
				break;
				
				case '3' : // Zwiekszanie poziome
					if(flg_wyb==0) { schp.hm -= 1*schp.hs; flg_chart_browse=1;   }	// Przesuwanie poziome
					else { 					 schp.hs  = schp.hs < 300 ? schp.hs+1 : 300; }
				break;
				
				case '5' :  // Zwiekszanie pionowe
					if(flg_wyb==0) { schp.vm += 1*schp.vs; } 
					else 					 { schp.vs = schp.vs > 1 ? schp.vs-1 : 1;  }		  // Skala pionowa
				break;
				
				case '2' :	// Zmniejszanie pionowe
					if(flg_wyb==0) { schp.vm -= 2*schp.vs; }	// Przesuwanie pionowe
					else 					 { schp.vs = schp.vs < 100 ? schp.vs+1 : 100; }
				break;
				
				case 'p':	// Zapisz biezace wyniki na karte 
					strB = DIMSDBLOCK*((hmt-schp.hb)/wXlen) + schp.sb;		// Blok zapis danych
					if(!mci_write_sect (strB,p_TSpm,DIMSDBLOCK)) 
					TFT_txt("Er0",0,6*14,RED); 
				...
				default: 
					data_looger_tool_menu();
					data_looger_tool( &sda, &schp, &chr);
					flg_chart_change = 0;
				break;	
				
			}
			
			if(flg_chart_browse)  schp.hb= wXlen/2; else schp.hb= wXlen;			
			
			BKP->DR1=schp.hm;	BKP->DR2=schp.hm>>16;
			BKP->DR3=schp.vm;
			BKP->DR4=schp.hs;
			BKP->DR5=schp.vs;
		...
		//================================================================================
		if( STCLK_MS*(100/DLS) < (trg0 - SysTick->VAL)){
			trg0 = SysTick->VAL;
			...
			//ADC1->SR      = 0;						// nalezy wyzerowac caly rejestr ADC1->SR
			//ADC1_SR_EOC   = 0;
			
			ADC1_CR2_ADON = 1;							// start conversion ADC
			Delay_200n(9);
			TSpm.dat[0].d4 = (ADC1->DR/14) ;
			Delay_200n(9);
			TSpm.dat[0].d5 = (ADC1->DR/14) ;
			
			schp.hs = 1 + (TSpm.dat[0].d4*TSpm.dat[0].d4)/300 ;
			
			if(TSpm.dat[0].d6 != schp.hs){
				TSpm.dat[0].d6 = schp.hs;
				flg_chart_change = 1;
			}
			
			TFT_txt(int2str(TSpm.dat[0].d4,str,3,0)	,0,10*14,RGB(3, 3, 0)); 
			TFT_txt(int2str(TSpm.dat[0].d5,str,3,0)	,0,9*14	,RGB(3, 3, 0));  
			
			//TFT_txt(hex2str(tim				,str,8)		,50 ,229	,YELLOW); // 
			TFT_txt(int2str(schp.dv[1] 	,str,5,0)	,50 ,229	,YELLOW); // 
			
			//TFT_txt(int2str(AMh						,str,3,1)	,0,3*14	,RED);    // 
			//TFT_txt(int2str(AMt						,str,3,1)	,0,2*14	,GREEN); 
		...
		if(f3_rtc_1sec){	// Aktualizacja zegara
			...
			if(((Rtc.sec+60*Rtc.min)%(schp.hs*DLD))==0){	flg_refresh = 1;	} // Odswierzanie wykresu
			if ((Rtc.sec%DLD                      )==0){	flg_wr_data = 1;	} // Zapis danych
			
			TFT_txt(tbuf,260,229,YELLOW);
			
			if(0){
				CopyDate(dbuf,&Rtc);
				TFT_txt(dbuf,0	,229,YELLOW);
			}
		}
		//================================================================================
		if(flg_wr_data ){	// pomiar co 5sek
			flg_wr_data = 0;
			
			///hmt = sch.hm+schp.hb*sch.hs + 1;
			
			hmt = ( ((RTC->CNTH<<16)|RTC->CNTL) - DATA_LOGGER_STIME)/DLD;
			x   =  hmt%wXlen;
			
			//if(Rtc.sec==0) { TSpm.dat[x].d3 = x; LED1 = 1;  }
			//else   				 { TSpm.dat[x].d3 = 0; LED1 = 0; }			
			...
			Delay_200n(10);
			TSpm.dat[x].d0 = (ADC1->DR/20) ;
			Delay_200n(10);
			TSpm.dat[x].d1 = (ADC1->DR/20) ;
			
			TIM6_stop(&tim);
			*/
			
			//if(x==0) TSpm.dat[x].d0 = 20; else  TSpm.dat[x].d0 = 0; 
			
			//TIM_start(); 
			if(!(DHT_read( &AMh, &AMt,1,&AM2302_pin)) ){		// AM2302 read T & H
				TSpm.dat[x].d1 = AMh;
				//TSpm.dat[x].d2 = AMt;
				TSpm.dat[x].d2 = AMt+200;
			}
			//TIM_stop(&tim); 
			//TFT_txt(hex2str(ADC1->SR,    str,4)	,60,229	,YELLOW); // 
			
			if(x==wXlen-1){
				
				strB = DIMSDBLOCK*((hmt)/wXlen) + schp.sb;		// Blok zapis danych
				TIM_start();
				if (mci_write_sect (strB,p_TSpm,DIMSDBLOCK)) 	{ LED2 ^= 1; 								 }
				else																					{	TFT_txt("Er0",0,6*14,RED); }
				TIM_stop(&tim);
			}			
			TFT_txt(int2str(x	,str,3,0)	,0,12*14	,YELLOW); // 
			
			
		}
		//================================================================================
		if(flg_refresh){	// odswierzanie wg. skali hs0
			flg_refresh=0;
			
			if(flg_chart_browse){				// odswierzaj
				kier=1; 									// wszystkie bloki z karty
				
			} else {
				///sch.hm += 1*sch.hs;		// przesun wykres
				schp.hm  = hmt ;					// przesuwaj wg. czasu
				flg_chart_change = 1;			// poka¿ wykres
				kier		= 0;							// ostatni blok jest pomiarowy					
			}
		}
		...
		if(flg_chart_change){ 
			flg_chart_change = 0;
			data_logger_chart(&sda,&schp,&schd,kier);
			
		}
	}
}//================================================================================

void data_logger_chart( T_SD1 *sd, T_CHART_PARAM *chp, T_CHART_DATA *chd,u08 sw2){  // Rysowanie wykresu z karty SD
		s32 yn[4],yp[4],M,m,strBLK;		
		
		u16 x,n=0,nrys=0;
		s16 px=0;		
		T_SD1 *p_s  = sd;
		u08 *p_sd = (u08 *)sd;	// wskaŸnik na strukture do odczytu z karty pamiêci
		u08 last_p=0,f0=1;
		T_RTC lrtc;
		u32 uxt;
		
		char str[10],tbuf[9]={"00:00:00"};
		sw2=sw2;
		
		if(flg_chart_browse)
		TFT_Fill(wXmin+143,wYmin,wXmin+143,wYmax,DGRAY);	// rysuj symetraln¹ pionowa (Bisector)
		else
		TFT_Fill(wXmin+143,wYmin,wXmin+143,wYmax,BLACK);	// rysuj symetraln¹ pionowa (Bisector)
		
		M = chp->hm-(chp->hb)*(chp->hs);		  		//bezwzglêdny index danych 'M' dla x=0
		m = M%wXlen;															//wzglêdny index danych
		strBLK = DIMSDBLOCK*(M/wXlen) + chp->sb;	//blok do odczytu
		
		uxt = RTC->CNTL+(RTC->CNTH<<16);
		
		/*
		UaPutS("\f hm,\t vm,\t  hs,\t vs,\t M,\t m,  T_SD1 \r\n");
		UaPutS("");		sint2uart(chp->hm); UaPutS("\t"); sint2uart(chp->vm);
		UaPutS("\t"); sint2uart(chp->hs); UaPutS("\t"); sint2uart(chp->vs);	
		UaPutS("\t"); sint2uart(M);				UaPutS("\t"); sint2uart(m);
		UaPutS("\r\n");
		*/
		if(strBLK<0) return;
		
		/*if(mci_read_sect (strBLK,p_sd,DIMSDBLOCK))	// pierwszy odczyt pamieci masowej
		{ strBLK += DIMSDBLOCK; } */
		
		for(x=0;x<(wXlen);x++){
			if (1){								// warunek wejœcia w zakres prawid³owych danych
				
				if(1){												// odczyt z pamieci flash (nowe wartosci)
					yn[0]=0;	yn[1]=0;					// pierwsze zerowanie zmiennych 
					yn[2]=0;	yn[3]=0;					// pomocniczych	punktu nastêpnego
					//memset (yn,0,4*4);
					
					for(n=0;n<chp->hs;n++){		// obliczenie œredniej dla jednego punktu ================
						
						if((m+n)>(wXlen-1) || f0){	// Odczytaj SD gdy przekroczony zakres
							
							if(++last_p <= chp->hs || sw2==1){	//ostatni wykres
								if(mci_read_sect (strBLK,p_sd,DIMSDBLOCK)){
									strBLK += DIMSDBLOCK;
									///UaPutC('.');
								} else { UaPutS("\r\n Blad odczytu"); while(1); }
							}else {
							  p_s = &TSpm;	// Dla ostatniego przebiegu zmiana na strukture pomiarowa
								///UaPutC(',');
						...
						// sumowanie wynikow dla danej skali =====================================
						
						if(chp->sc & (1<<0)) yn[0] += p_s->dat[m+n].d0;
						if(chp->sc & (1<<1)) yn[1] += p_s->dat[m+n].d1;
						if(chp->sc & (1<<2)) yn[2] += p_s->dat[m+n].d2;
						if(chp->sc & (1<<3)) yn[3] += p_s->dat[m+n].d3;
						
					}
					m += chp->hs;
					
					for(u08 k=0;k<NChart;k++){			// wybierz wykresy do wyswietlania
						if(chp->sc & (1<<k)){					// Wybór wykresu
							
							
							// Wartosc w symetralnej
							if( x == chp->hb-1 ){
								chd->bv[k] = yn[k]/chp->hs ;
								//mid_scl = ((strBLK - chart->sb)/DIMSDBLOCK)*(wXmax-wXmin) + m + n/2;																
								//mc = m+n;
							} 
							yn[k]/= chp->hs;
							yn[k] = (chp->ml[k]*(yn[k] + chp->ad[k]))/chp->dv[k];	// obliczenie dla punktu
							yn[k] = wYmin + VerB + (chp->vm + yn[k])/chp->vs;     // Obl wsp. punktu wyswietlacza	
						}
					}
					chd->bv[2] -= 200;
					
				} else {	// odczyt z bufora (wartosci z wykresu)
				...
				for(u08 k=0;k<NChart;k++){					// wybierz wykresy do wyswietlania
					if(chp->sc & (1<<k)){						// Wybór wykresu
						...
						// Czysc poprzedni punkt
						if(x!=wXlen-1){
							TFT_dy (x+wXmin ,chd->bc[k][x] ,chd->bc[k][x+1] ,BLACK,chp->wd[k]); 												
						}
						
						if ((yn[k]+chp->wd[k]) > wYmax) yn[k]= wYmax;				// Ograniczenie górne
						if (yn[k] < wYmin) 							yn[k]= wYmin;				// Ograniczenie dolne
						
						//Rysuj punkt wykresu
						if(x!=0){
							TFT_dy (x-1+wXmin ,yp[k] ,yn[k] ,chp->cc[k],chp->wd[k]); 
							nrys++;
						}
						chd->bc[k][x] = yn[k];	// Zapisz wsp. punktu do bufora wykresu
						yp[k]					= yn[k];	// Zapisz wartoœæ poprzedni¹
					}
				}
			}
			//sint2uart(m);
		...
		for(u08 k=0;k<NChart;k++){							
			
			if(chp->sc & (1<<k))	TFT_txt(int2str (chd->bv[k],str,3 ,chp->dp[k]),0,14*k*2 ,chp->cc[k]);
		}
		
		TFT_txt(int2str(chp->hs ,str,3,0)	,0	,13*14	,YELLOW);	// Skala
		//TFT_txt(int2str(chart->hm		,str,4,0)	,0	,14+4*14 	,YELLOW);
		//TFT_txt(int2str(M						,str,5,0)	,0	,0 				,YELLOW);
		//TFT_txt(int2str(chart->hm,str,6,0)	,0,0				,YELLOW); // srodek skali
		
		if(flg_chart_browse==0) TFT_txt("ONL",0,15*14,RGB565CONVERT(255, 0, 0 )); 
		else 			 							TFT_txt("OFL",0,15*14,YELLOW); 
		if(flg_wyb==0) 					TFT_txt("MOV",0,15*13,YELLOW); 
		else 			 							TFT_txt("SCL",0,15*13,GREEN); 
		
		uxt=DLD*chp->hm + DATA_LOGGER_STIME-2;
		CopyTime(tbuf,rtc_unix2time(uxt, &lrtc)); 
		TFT_txt(tbuf,150,0,YELLOW); // czas w punkcie srodkowym
		
	}

void data_looger_tool_menu( void){
		UaPutK(" ================= Data Looger Tool ===============\r\n");
		UaPutK("  a - add data  r - Read  data  i - Import data    \r\n");
		UaPutK("  z - write raw w - Write data  s - reset & time	 \r\n");
		UaPutK(" ==================================================\r\n > ");
	}
void data_looger_tool( T_SD1 *sd,T_CHART_PARAM *chart, char *ch){
		char  chr=*ch ,sget[20],snum[8];
		u08 ws;
		u16 crc16,g;
		s16 b,f;
		u32 hm,avg,stB,strBlock,tim=0;
		static T_SD0	 sdb;			// Odczyt przez port szeregowy z innego urzadzenia
		
		//u32 
		//chart->hm
		
		
		u08 *p_sd  = (u08 *)sd; // 8bitowy wskaznik na tablice odczytu/zapisu karty pamieci
		//u16 *p_crc = (u16 *)sd;
		u08 *p_sdb = (u08 *)&sdb;
		...
		switch (*ch) {
			...
			break;
			//======== Zapis testowych danych na krte SD ====================
			case 'r':
				UaPutK("\r\n Index HorBisector = ");
				hm=UART_getNum();
				strBlock = DIMSDBLOCK*(hm/1) + chart->sb;	// Blok odczytu
				b = hm%wXlen;
				
				UaPutK("\r\n Sector memory = ");
				sint2uart(strBlock);
				
				UaPutK("\r\n Time    \t d0   d1   d2   d3   d4   d5   d6   d7   d8   e0   e1\r\n\r\n");
				if(mci_read_sect (strBlock,p_sd,DIMSDBLOCK)){
					for (f=b-8>0?b-8:0;f<b+8 ;f++){
						
						num2uart(hm-b+f,8,0); 
						num2uart(f,4,0); UaPutK(" -");
						num2uart(sd->dat[f].d0,5,0); 
						num2uart(sd->dat[f].d1,5,0); 
						num2uart(sd->dat[f].d2,5,0); 
						num2uart(sd->dat[f].d3,5,0); 
						num2uart(sd->dat[f].d4,5,0); 
						num2uart(sd->dat[f].d5,5,0); 
						num2uart(sd->dat[f].d6,5,0); 
						num2uart(sd->dat[f].d7,5,0); 
						num2uart(sd->dat[f].d8,5,0); 
						num2uart(sd->dat[f].e0,5,0); 
						num2uart(sd->dat[f].e1,5,0); 
						UaPutK("\r\n");
					}
					//crc16_spi2(p_crc,DIMSDBLOCK*256, &crc16 );
					//UaPutK("\r\n crc2 ");	
					//uint2uart(2*DIMSDBLOCK*256);
					//UaPutK(" byte =");	
					//hex2uart(crc16,4);
				}
				strBlock += DIMSDBLOCK;
			...
			break;
			case 'w': //Write data all
				
				UaPutK(" \r\n Podaj G_index  = ");		hm = UART_getNum();
				UaPutK(" \r\n wartosc        = ");		g  = UART_getNum();
				
				strBlock = DIMSDBLOCK*(hm/wXlen) + chart->sb;	//blok do odczytu
				f = hm%wXlen;
				...
				if(mci_read_sect (strBlock,p_sd,DIMSDBLOCK)){
					...
					if(mci_write_sect (strBlock,p_sd,DIMSDBLOCK)){
						
						UaPutS("\r\n Write OK ");
						num2uart(strBlock,5,0); 
						num2uart(f,5,0); 
						//data_logger_chart(sd,chart,1);
					}
					else { UaPutS("\r\n Write SD ERROR");	}
				}
			...
			break;
			case 'z': //Write data raw
				
				UaPutK("\r\n Wykres start = ");	strBlock=UART_getNum()*DIMSDBLOCK+chart->sb;
				UaPutK("\r\n Ile Wykresow = ");	b=UART_getNum();
				UaPutK("\r\n liczba       = ");	g=UART_getNum();
				while(b--){
			...
			case 'i':
				...
				for(u16 z=0;z<g*8;z++){
					//UaPutS2("t");
					//UaPutS2(uint2str(strBlock,snum));
					//UaPutS2("\xd""1""\xd");
					//UaPutS2("t1000""\xd""1""\xd");
					//Delay_us(200);
					TIM_start(); 
					do{
						strcpy (sget,"t");
						strcat (sget, (const char*) uint2str(strBlock,snum) );
						strcat (sget,  "\xd""1""\xd");
						ws = strlen (sget);
						UaPutS2(sget);
						avg  = 0;
						...
						while (avg < (u32)(512+ws)){
							...
							if(STCLK_MS*60 < (trg0 - SysTick->VAL) ){ break; }
						}
					}while(avg-ws != 512);
					strBlock ++;
					for(u08 s=0;s<36;s++){
						sd->dat[f].d0 = sdb.dat[s].d0;
						sd->dat[f].d1 = sdb.dat[s].d1;
						sd->dat[f].d2 = sdb.dat[s].d2;
						sd->dat[f].d3 = sdb.dat[s].d3;
						f++;
						if(f==288){
							if(mci_write_sect (stB,p_sd,DIMSDBLOCK)){
								UaPutS("Write OK \r\n");
								stB += DIMSDBLOCK;
							}
							else { UaPutS("Write SD ERROR \r\n");	}
							f=0;
						}
					}
					//LED1=0;
					TIM_stop(&tim); 
					crc16_spi2((u16 *)p_sdb,256, &crc16 );
					if(avg==0){
					}
					//TIM_stop(&tim); 
					//LED1=0;
			...
			case 's' :	
				...
				UART_getStr(snum);
				if(snum[0]=='t'){
					UaPutK("\r\n Podaj rok [yyyy] = ");		Rtc.yer=UART_getNum();	// 1970..2106 
					UaPutK("\r\n Podaj mon [1..12]= ");		Rtc.mon=UART_getNum();	// 1..12 
					UaPutK("\r\n Podaj mdy [1..31]= ");		Rtc.mdy=UART_getNum();	// 1..31 
					UaPutK("\r\n Podaj hor [0..23]= ");		Rtc.hor=UART_getNum();	// 0..23 
					UaPutK("\r\n Podaj min [0..59]= ");		Rtc.min=UART_getNum();	// 0..59 
					UaPutK("\r\n Podaj sec [0..59]= ");		Rtc.sec=UART_getNum();	// 0..59 
					UaPutK("\r\n Podaj wdy [0..6] = ");		Rtc.wdy=UART_getNum();	// 0..6 (Niedziela..Sobota) 
					rtc_SetTime(&Rtc);
				}
				
			break;
			case 't':
				hm = RTC->CNTL + (RTC->CNTH<<16);
				UaPutK("\r\n UXT=");	sint2uart(hm); 
				Rtc.yer=2018;	// 1970..2106 
				Rtc.mon=2;	// 1..12 
				Rtc.mdy=15;	// 1..31 
				Rtc.hor=7;	// 0..23 
				Rtc.min=0;	// 0..59 
				...
				rtc_time2unix ( &avg, &Rtc);
				//rtc_SetTime(&Rtc);
				
				UaPutK("\r\n DATA_LOGGER_STIME=");	sint2uart(avg); 
				UaPutK("\r\n hm =");	sint2uart((hm-avg)/5); 
				
			break;
			case 'a':
				UaPutK("\r\n Podaj Wartosc = ");	trg5 = UART_getNum();
				trg5 = trg5 - 10000;
				UaPutK("\r\n Wartosc  = ");				uint2uart(trg5); 
				UaPutK("\r\n Wartosc  = ");				sint2uart(trg5); 
				
				//UaPutK("\r\n Wskaznik = ");				hex2uart((u32)p_ws,8); 
				UaPutK("\r\n Wskaznik = ");				hex2uart((u32)&BKP->DR20,8); 
				//UaPutK("\r\n Podaj Wskaznik = ");	p_ws = UART_getHex();
				//hm = UART_getNum();
...
void ramka(void){
		...
		TFT_Fill(wXmin-1,wYmin-1,wXmax+1,wYmin-1,LGRAY);	// pozioma dolna
		TFT_Fill(wXmin-1,wYmax+1,wXmax+1,wYmax+1,LGRAY);	// pozioma górna
		TFT_Fill(wXmin-1,wYmin-0,wXmin-1,wYmax+0,LGRAY);	// pior_dma_bufa lewa
		TFT_Fill(wXmax+1,wYmin-0,wXmax+1,wYmax+0,LGRAY);	// pior_dma_bufa prawa
	}
		...
		*/
		...
		while(1){
			...
			if( STCLK_MS*500 < (trg1 - SysTick->VAL) ){
				...
				//TFT_txt(int2str(x,str,1,0),0,0, YELLOW); // œrodek skali 	x
				//TFT_txt(int2str(x,str,2,0),0,10, YELLOW); // œrodek skali 	x
...
void ADC1_DMA_Tim4_Init(void)		  {		
    ...
    TIM4->CCR4  = 10;		//width pulse on PB9 out channel
		...
		//Kana³y czytane w odwrotnej kolejnoœci do parametru "sequence"
		ADC1->SQR1  = 0<<20;
