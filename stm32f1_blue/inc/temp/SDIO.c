...
#define SD2UM         (0x02)
#define mmc_Ena()	GPIOC->BSRRH = (1<<11); //CS Low
#define mmc_Dis()	GPIOC->BSRRL = (1<<11); //CS High
...
void SD_LowLevel_Init(void) {
  ...
  //1	-	CS			- PC11
	...
	//gpio_pin_cfg(GPIOC, 8,  GPIO_AF12_PP_25MHz_PULL_UP);	//D0
	//gpio_pin_cfg(GPIOC, 9,  GPIO_AF12_PP_25MHz_PULL_UP);	//
	//gpio_pin_cfg(GPIOC, 10, GPIO_AF12_PP_25MHz_PULL_UP);	//
	//gpio_pin_cfg(GPIOC, 11, GPIO_OUT_PP_25MHz);	//CS
	//gpio_pin_cfg(GPIOC, 12, GPIO_AF12_PP_25MHz_PULL_UP);	//CLK
	//gpio_pin_cfg(GPIOD, 2,  GPIO_AF12_PP_25MHz_PULL_UP);	//DI/CMD
  
	GPIOC->CRH &= 0x000fffff;//PA4~7
		GPIOC->CRH =
		m_GPIO_MODE_CNF(0, GPIO_CRx_MODE_OUT_10M_value, GPIO_CRx_CNF_ALT_PP_value) | 
		m_GPIO_MODE_CNF(1, GPIO_CRx_MODE_OUT_10M_value, GPIO_CRx_CNF_ALT_PP_value) | 
		m_GPIO_MODE_CNF(2, GPIO_CRx_MODE_OUT_10M_value, GPIO_CRx_CNF_ALT_PP_value) |
		m_GPIO_MODE_CNF(3, GPIO_CRx_MODE_OUT_10M_value, GPIO_CRx_CNF_ALT_PP_value) |
		m_GPIO_MODE_CNF(4, GPIO_CRx_MODE_OUT_10M_value, GPIO_CRx_CNF_ALT_PP_value);
	
	GPIOD->CRL &= 0x00000f00;//PA4~7
		GPIOD->CRL =
		m_GPIO_MODE_CNF(2, GPIO_CRx_MODE_OUT_10M_value, GPIO_CRx_CNF_ALT_PP_value); 
		...
		tempreg|=SDIO_CLKCR_CLKEN; //| SDIO_CLKCR_PWRSAV;  //Clock is enabled
...
void SD_Init(void){
  ...
  uint32_t tempreg;
	...
	//	SDIO->ARG = 0xffffffff;
	//	//SD_Command(0x60,NORESP ,0xffffffff);
	//}
	//mmc_Ena();
  ...
  SD_Response(&response, RESP_R7); 
	//AA is the check pattern. If response does not match with it, execution will be blocked in panic  
  
  while (1) {
    ////Send ACMD41    
    //CMD55
    SD_Command(CMD55, SHRESP, 0); 
		//Note that argument should be RCA. But at this point RCA of SD is 0. (It will be changed after cmd3)
...
static uint32_t SD_Command(uint32_t cmd, uint32_t resp, uint32_t arg) {
		...
		if (resp==NORESP) {
			//We should wait for CMDSENT or CTIMEOUT
			//TIM6->CNT  = 0;		
			while (!(SDIO->STA & (SDIO_STA_CTIMEOUT | SDIO_STA_CMDSENT))) {};
			//RTC->BKP0R = (TIM6->CNT);		
		} 
		else {//SHRESP or LNRESP or R3RESP
			//We should wait for CMDREND or CCRCFAIL or CTIMEOUT			
...
static void SD_Panic(uint32_t code, char *message) {
  ...
  // Block the execution with blinky leds
	switch (code)		{			
			case CMD0  : led = 12;	break;	//Green
			case CMD8  : led = 13;	break;	//Orange
			case CMD55 : led = 14;	break;	//Red
			default 	 : led = 15;	break;	//Blue
		}
  while (1) {
...
void SD_ReadSingleBlock(uint8_t *buf, uint32_t blk) {
		uint32_t __attribute__((unused)) ReadAddr;
		uint32_t response;
...
void SD_StartMultipleBlockWrite(uint32_t blk) {
  uint32_t __attribute__((unused)) WriteAddr;
  uint32_t response;
...
static void SD_StartBlockTransfer(uint8_t *buf, uint32_t cnt, uint32_t dir){
		...
		DMA2_Channel3->CPAR=((uint32_t) 0x40018080);  //SDIO FIFO Address (=SDIO Base+0x80)
		DMA2_Channel3->CMAR=(uint32_t) buf;    //Memory address
		
		//Set the number of data to transfer
		DMA2_Channel3->CNDTR=0;   //Peripheral controls, therefore we don't need to indicate a size
		...
		DMA2_Channel3->FCR=tempreg;
		
		//Set the Direction of transfer
		if (dir==UM2SD) {
			DMA2_Channel3->CR|=(0x01<<6) & DMA_SxCR_DIR;
		} 	
		else if (dir==SD2UM) {
			DMA2_Channel3->CR|=(0x00<<6) & DMA_SxCR_DIR;
		}
		
		//Enable the DMA (When it is enabled, it starts to respond dma requests)
		DMA2_Channel3->CR|=DMA_SxCR_EN;
		//END of PART I
...
void SD_WaitTransmissionEnd(void) {
		...
		while (DMA2_Channel3->CR & DMA_SxCR_EN) {};
